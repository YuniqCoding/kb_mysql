-- sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl1 을 생성하고 ,
...
-- sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl2 를 생성하고 ,
-- 자동 생성된 인덱스 목록을 확인하세요
CREATE TABLE tbl2 (
...
-- sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl3 를 생성하고 ,
-- 자동 생성된 인덱스 목록을 확인하세요
CREATE TABLE tbl3(
	a INT UNIQUE,
    b INT UNIQUE,
...
-- sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl4 를 생성하고 ,
-- 자동 생성된 인덱스 목록을 확인하세요
CREATE TABLE tbl4(
...
-- sqldb 데이터베이스에 다음과 같은 컬럼을 가지는 테이블 tbl5 를 생성하고 ,
-- 자동 생성된 인덱스 목록을 확인하세요
CREATE TABLE tbl5(
	a INT UNIQUE NOT NULL,
    b INT UNIQUE,
...
-- testdb에 다음 컬럼 목록을 가지는 usertbl 을 만드세요
...
-- ALTER를 사용하여 usertbl 에서 PRIMARY KEY 제약조건을 제거하고
